...
public class SendAllMessagesServlet extends BaseServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		String userMessage = "Empty Message";

		try {
			userMessage = req.getHeader(SendMessageServlet.PARAMETER_MESSAGE);
		} catch (Exception e) {
			e.printStackTrace();
			return;
		}
					...
					if (partialSize == Datastore.MULTICAST_SIZE
						...
						TaskOptions taskOptions = TaskOptions.Builder
								...
								.param(SendMessageServlet.PARAMETER_MULTICAST,
										multicastKey)
								.param(SendMessageServlet.PARAMETER_MESSAGE,
										userMessage).method(Method.POST);
						queue.add(taskOptions);
		...
		req.setAttribute(CreateBetServlet.ATTRIBUTE_STATUS, status.toString());
